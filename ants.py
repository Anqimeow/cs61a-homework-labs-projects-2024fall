    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if place.ant.can_contain(self):
                ...
                raise AssertionError('Too many ants in {0}'.format(place))
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while place is not None:
            if place.is_hive:
                return None
            elif distance>=self.lower_bound and distance<=self.upper_bound:
                if place.bees:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt provides a large amount of health to block Bees."""
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        ...
        if other.is_container:
            return False
        elif self.ant_contained is not None:
            return False
        else:
            return True
        ...
        while current_place is not None:
            ...
            while ant is not None:
                ...
                if ant.is_container and ant.ant_contained is not None:
                    if not hasattr(ant.ant_contained, 'doubled'):
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        ...
        #确保original_action仅在蜜蜂首次被减速时保存其真正的原始action方法，后续再次被攻击时不会被slow_action覆盖
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        if isinstance(target,Bee) and not target.hasbeen_scared:
            target.scare(2)
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        while True:
            ...
            if current_place is None or getattr(current_place, 'is_hive', False):
                break
            ...
            while ant is not None:
                ...
                # 递归收集容器内的昆虫（如Bodyguard包含的蚂蚁）
                ant = ant.contained_ant if hasattr(ant, 'contained_ant') else None
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if self.scared_turns > 0:
            ...
            if back_place is not None and not back_place.is_hive:
                self.move_to(back_place)
                self.scared_turns -= 1  
                return  
    ...
    def scare(self,length):
        ...
        # BEGIN Problem EC 2
        "*** YOUR CODE HERE ***"
        if not self.hasbeen_scared:
            self.hasbeen_scared= True
