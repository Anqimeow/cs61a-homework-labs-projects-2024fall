def reverse_list(lst):
    """Reverse a Scheme list (Pairs/nil) and return the new head."""
    result = nil
    while lst is not nil:
        result = scheme_cons(lst.first, result)
        lst = lst.rest
    return result
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        current_formal = formals
        while current_formal is not nil:
            param = current_formal.first
            if not scheme_symbolp(param):
                raise SchemeError(f'invalid formal parameter: {param}')
            current_formal = current_formal.rest
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        # Each binding must be a list of length 2: (symbol expr)
        validate_form(binding, 2, 2)
        symbol = binding.first
        expr = binding.rest.first
        names = scheme_cons(symbol, names)
        vals = scheme_cons(scheme_eval(expr, env), vals)
        bindings = bindings.rest
    # Reverse the lists to preserve order
