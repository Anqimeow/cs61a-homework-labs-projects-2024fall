...
def pick(paragraphs, select, k):
    ...
    for _ in range(k):
        next(selected_paragraphs, '')
    return next(selected_paragraphs, '')
...
def about(subject):
    ...
    def select(paragraph):
        paragraph_words = split(lower(remove_punctuation(paragraph)))
        for word in subject:
...
def furry_fixes(typed, source, limit):
    ...
    def helper(typed,source,limit,diff):
        if diff>limit:
            return diff
        elif len(typed)==0 or len(source)==0:
            return diff+abs(len(typed)-len(source))
        elif typed[0]==source[0]:
            return helper(typed[1:],source[1:],limit,diff)
        else:
            return helper(typed[1:],source[1:],limit,diff+1)
    ...
    for w in word_indices:
        ...
        for p in player_indices:
            if times[p][w]<min_time:
                min_time=times[p][w]
                fastest_player=p
        fastest[fastest_player].append(words[w])
    return fastest
    # END PROBLEM 10
