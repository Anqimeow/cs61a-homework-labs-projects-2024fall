...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in scheme_forms.SPECIAL_FORMS:
        ...
        evaluated_args = rest.map(lambda operand: scheme_eval(operand, env))
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        try:
            ...
            while current is not nil:
                python_args.append(current.first)
        ...
        # BEGIN PROBLEM 11
        # A MuProcedure uses dynamic scope: the parent of the new call frame
        # should be the environment in which the call expression was evaluated
        # (the current env), not the environment where the mu was defined.
